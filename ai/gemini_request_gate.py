"""One-shot gate for explicit real Gemini requests."""
...
class GeminiRequestGate:
    """Allow exactly one explicit Gemini request without changing global state."""
    ...
    def __init__(
        ...
        self.router = router
        self.provider_factory = provider_factory or GeminiProvider
    ...
    def can_make_real_request(self) -> GeminiRequestGateStatus:
        ...
        if status is None:
            return GeminiRequestGateStatus(
                provider_configured=False,
                key_status=AIProviderKeyStatus.MISSING,
                default_provider_name=default_name,
                can_request=False,
                reason="Gemini provider config is missing.",
            )
        credential_status = self.credential_runtime.credential_status("gemini")
    ...
    def generate_one_shot(
        ...
        credential = self.credential_runtime.resolve_credential("gemini")
        if not credential.safe_to_use:
            return self._error_response(capability, "GEMINI_API_KEY is missing.")

        config = self._temporary_enabled_gemini_config(guard_result.model)
        ...
        if response.is_error:
            return self._error_response(
                capability,
                response.error_message or response.text or "Gemini request failed.",
            )
        return response
    ...
    def status_text_ru(self) -> str:
        ...
        return "\n".join(
            [
                "Gemini one-shot real request status:",
                ...
                f"- maxOutputTokens: {self.request_guard.config.max_output_tokens}",
    ...
    def _temporary_enabled_gemini_config(self, model: str) -> AIProviderConfig:
        config = self.config_manager.get_config("gemini")
        if config is None:
    ...
    def _error_response(capability: AIProviderCapability, message: str) -> AIResponse:
        return AIResponse(
            text=message,
            provider_name="gemini_request_gate",
            model_name="none",
