from ai import GeminiRequestCostGuard, GeminiRequestGuardConfig


def test_default_model_resolves_safely():
    guard = GeminiRequestCostGuard(environ={})
...
def test_gemini_model_env_var_overrides_model():
    guard = GeminiRequestCostGuard(environ={"GEMINI_MODEL": "gemini-2.5-flash"})

    result = guard.guard_request("hello")

    assert result.allowed is True
    assert result.model == "gemini-2.5-flash"
    assert guard.model_source() == "GEMINI_MODEL"


def test_invalid_model_rejected():
    too_long = "m" * 101
    for model in ("", "bad model", "bad/model", "bad\\model", "AIzaSecretKey", too_long):
        guard = GeminiRequestCostGuard(environ={"GEMINI_MODEL": model})
...
def test_prompt_empty_rejected():
    result = GeminiRequestCostGuard(environ={}).guard_request(" ")
...
def test_prompt_over_max_chars_rejected():
    guard = GeminiRequestCostGuard(
        GeminiRequestGuardConfig(max_prompt_chars=5),
        environ={},
    )
...
def test_warning_mentions_free_tier_quota_without_key_leakage():
    secret = "fake-gemini-key-that-must-not-appear"
    guard = GeminiRequestCostGuard(environ={"GEMINI_API_KEY": secret})

    result = guard.guard_request("hello")

    assert result.warning_text is not None
    assert "free tier" in result.warning_text
...
def test_invalid_model_value_is_not_printed_in_status_or_model_text():
    secret_like_model = "AIzaSecretThatMustNotAppear"
    guard = GeminiRequestCostGuard(environ={"GEMINI_MODEL": secret_like_model})
